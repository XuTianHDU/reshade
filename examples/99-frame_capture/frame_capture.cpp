...
// 这个类包含了resource，resource_desc，resource_view三个资源
...
{
	// 初始化的时候如果配置文件中有这三个参数的值就覆盖，没有就是默认值
...
{
	// 在runtime中加入stored_buffers_inst这个对象的类似buffer的东西，能随时 set 和 get
...
{
	// 拿到指向 private_data 中的 stored_buffers_inst 和 device
	stored_buffers_inst &sbi = runtime->get_private_data<stored_buffers_inst>();
	device *const device = runtime->get_device();

	// 每次渲染完成之后拿到上一帧对应的纹理对象 ExportTex 存到 private data中
	runtime->enumerate_texture_variables(nullptr, [&sbi, &device](effect_runtime *runtime, auto variable) {
		char source[32] = "";
		// 拿到遍历的纹理的名字并将其赋给 source 变量
		...
		if (std::strcmp(source, "DepthToAddon_ExportTex") == 0) {
			...
			if (srv != 0) {
				// 通过resource view 拿到resource，resource description并且更新 private data 中的这三个值
...
static void on_reshade_present(effect_runtime *runtime)
{
	...
	if (runtime->is_key_pressed(0x79) && enableCapturing)
	{
		...
		// 获取当前文件的路径并且将其存储到 file_prefix 中
		GetModuleFileNameW(nullptr, file_prefix, ARRAYSIZE(file_prefix));

		// 将当前文件夹的信息赋给save_path，并且在后面加一个空格
		...
		// 将当前的时间信息加到路径上去，最后得到 C:\Games\Example\game.exe 2025-03-02 15-30-45 123 
		char timestamp[21];
...
{
	// 初始化的时候先创建一个用户交互界面,然后交互界面有三个值分别是enableCapturing,enableDepthExp,enableNormalExp
	// 可以在界面上交互来改变这三个变量的值，并且将其存到全局变量中 reshade.ini
	reshade::register_overlay("Frame Capture", draw_settings_overlay);

	reshade::register_event<reshade::addon_event::init_device>(on_init_device);
	// effect_runtime 是ReShade的核心引擎实例，代表了一个完整个的效果处理环境，可以访问ReShade的大部分功能
	// 是 addon 与 reshade 核心功能之间的桥梁
	// on_init_effect_runtime => 初始化runtime对象
	reshade::register_event<reshade::addon_event::init_effect_runtime>(on_init_effect_runtime);
	reshade::register_event<reshade::addon_event::destroy_effect_runtime>(on_destroy_effect_runtime);

	// reshade_present是在shade元素构建好显示到屏幕上前作的动作
